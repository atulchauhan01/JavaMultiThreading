 ...
 * calls being synchronized on the "same" object). The reason for this (other
 * than the standard thread safety concerns) is due to something known as a
 * missed signal. <br>
 * <br>
 * An example of this, is that a thread may call
 * {@link WaitAndNotify_8.BlockingQueue#put} when the queue happens to be full,
 * it then checks the condition, sees that the queue is full, however before it
 * can block another thread is scheduled. This second thread then
 * {@link WaitAndNotify_8.BlockingQueue#take()}'s an element from the queue, and
 * notifies the waiting threads that the queue is no longer full. Because the
 * first thread has already checked the condition. However, it will simply call
 * {@code wait()} after being re-scheduled, even though it could make progress.
 * <br>
 ...
 * also have to acquire lock before waking. <br>
 * <br>
 ...
 * case, the connection list). When our thread wakes up, it automatically
 * regains the lock. We can now check again that the list is not empty, and if
 * it isn't, safely take out the first connection. This checking and removing
 * will be atomic because we have the lock on the list.
 *
 *
